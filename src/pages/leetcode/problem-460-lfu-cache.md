---
title: LeetCode Problem 460 - LFU Cache
date: "2018-05-21"
description: Implement an LFU Cache
---
<paragraph>
    ...
    public int get(int key) {
        ...
        if (nodeMap.containsKey(key)) {
            ...
            if (retrievalMap.containsKey(node.retrievals)) {
                ...
                retrievalMap.put(node.retrievals, new LinkedList<Node>(){{add(node);}});
            }
            return node.val;
        }
        return -1;
...
<paragraph>
If an object does not exist for that key, the we will need to create a new Node for that key, add that node to the nodeMap, and add the Node to retrievalMap in the LinkedList where the key is 0.
</paragraph>
